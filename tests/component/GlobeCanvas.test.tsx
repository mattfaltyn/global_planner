...
import { getStopContextPointOfView } from "../../lib/globe/camera";
import { globeColors } from "../../lib/globe/colors";
import { getItineraryFitPointOfView } from "../../lib/state/selectors";
...
describe("GlobeCanvas", () => {
  ...
  it("uses itinerary-fit playback framing and derives active, future, and selected path styling", () => {
    const { stops, legs } = createResolvedFixtureItinerary();
    ...
    expect(globeApi.pointOfView).toHaveBeenCalledWith(
      getItineraryFitPointOfView(stops),
      900
    );
  ...
  it("renders the traveler marker and distinguishes active and visited stops", () => {
    ...
    expect(points).toHaveLength(stops.length + 1);
    expect(points.at(-1)?.kind).toBe("traveler");
    expect(pointAltitude(points.at(-1) as object)).toBeGreaterThan(0);
    ...
    expect(pointRadius(points.at(-1) as object)).toBe(0.18);
  });
